...
class Cross:Weapon ("Cross", "Jesus Smith De'") {
    override val damage: Int = 15
    override val range: Float = 500f
    override var attackDelay: Float = 5f
    ...
    private val texture = Texture("assets/items/weapons/Cross.png")
    private var projectilePosition: Vector2? = null
    private var targetEnemy: Enemy? = null
    private val projectileSize = Vector2(30f, 30f)
    ...
    override fun attack(target: Enemy, delta: Float, player: Player) {
        ...
        if (isRangedAttackActive) {
            ...
            // Trigger the Cross if the attack delay has passed and the target is in range
    ...
    private fun moveProjectileTowardsTarget(delta: Float) {
        ...
        targetEnemy?.let { enemy -> // targetEnemy renamed to enemy in the lambda
            projectilePosition?.let { position -> // projectilePosition renamed to position in the lambda
                // calculates the line direction from the Cross to the enemy.
                // Vector2(enemy.position) creates a new vector at the enemy's position.
                // sub(position) subtracts the Cross's position from this vector  resulting in a vector pointing from the Cross to the enemy
                // .nor() normalizes this vector to have a length of 1, which is useful for ensuring that the Cross moves consistently towards the enemy, no matter how far away the enemy is.
                val direction = Vector2(enemy.position).sub(position).nor()
                val speed = 200 // speed of the Cross
                // updates the Cross's position depending on speed and last rendered delta frame.
                position.add(direction.scl(speed * delta))
            }
        }
    }

    // Just draws the Cross
