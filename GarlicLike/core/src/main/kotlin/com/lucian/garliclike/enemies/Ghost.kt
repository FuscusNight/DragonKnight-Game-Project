...
class Ghost : Enemy {
    override var healthPoints: Int = 20
    override var damage: Int = 5
    private val speed = 60f

    private val texture1 = TextureRegion(Texture("assets/characters/GhostAnim1.png"))
    private val texture2 = TextureRegion(Texture("assets/characters/GhostAnim2.png"))
    override var textureRegion: TextureRegion = texture1
    override val width: Float = 32f
    override val height: Float = 42f
    ...
    // Updated so it sens a unique vector position for each ghost to avoid causing issues when using a spawn point system
    // otherwise it somehow would glue the spawn point vector position onto an enemy that spawn turning the enemy into a mobile spawn
    // That also somehow combines all of its stats with all respawning enemies creating royded up enemy that moves at super sonic speed
    ...
    override fun takeDamage(amount: Int) {
        healthPoints -= amount
        println("Ghost took $amount damage. Health now: $healthPoints")
        if (healthPoints <= 0) {
            dead = true
            if (dead)
            {
                println("Ghost is DEAD!")
            }
