...
class Sword:Weapon ("Sword", "Pointy") {
    override val damage: Int = 25
    override val range: Float = 95f
    ...
    private val texture = Texture("assets/items/weapons/Sword.png")
    private var projectilePosition: Vector2? = null
    private var targetEnemy: Enemy? = null
    private val projectileSize = Vector2(40f, 40f)
    ...
    override fun attack(target: Enemy, delta: Float, player: Player) {
        ...
        if (isRangedAttackActive) {
            ...
            // Trigger the sword if the attack delay has passed and the target is in range
    ...
    private fun moveProjectileTowardsTarget(delta: Float) {
        ...
        targetEnemy?.let { enemy -> // targetEnemy renamed to enemy in the lambda
            projectilePosition?.let { position -> // projectilePosition renamed to position in the lambda
                // calculates the line direction from the Sword to the enemy.
                // Vector2(enemy.position) creates a new vector at the enemy's position.
                // sub(position) subtracts the Sword's position from this vector  resulting in a vector pointing from the Sword to the enemy
                // .nor() normalizes this vector to have a length of 1, which is useful for ensuring that the Sword moves consistently towards the enemy, no matter how far away the enemy is.
                val direction = Vector2(enemy.position).sub(position).nor()
                val speed = 200 // speed of the Sword
                // updates the Sword's position depending on speed and last rendered delta frame.
